## If you want to convert the in memory db to actual postgres db using dbeaver, follow the below steps:

# Part 1 — PostgreSQL Schema (run this in DBeaver)

-- Run this in DBeaver on your database
...
-- PROJECT MEMBERS  (replaces the memberIds[] array)
...
CREATE TABLE issues (
  ...
  checklist   JSONB NOT NULL DEFAULT '[]',  -- [{id, text, done}]
...
# Part 2 — Files to Change in the Backend
File 1: backend/.env — add the DB connection string

PORT=4000
DATABASE_URL=postgresql://your_user:your_password@localhost:5432/pulseboard
...
File 2: backend/package.json — add the pg driver

npm install pg

File 3: backend/src/data/db.js — full rewrite

Replace the entire file with a PostgreSQL client + the same exported functions:
...
Files 4–8: All 5 controllers — add await + async

Since all db.* calls are now async, every controller function needs await. Here's the diff summary per file:

auth.controller.js — already async, just add await:
...
projects.controller.js — make functions async, add await:
...
issues.controller.js — same pattern on all 5 functions

users.controller.js — same pattern

comments.controller.js — same pattern


## The frontend doesn't know or care — it still talks to the same REST API endpoints with the same JSON shapes.